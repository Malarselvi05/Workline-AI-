...
@router.get("", response_model=List[WorkflowResponse], dependencies=[require_viewer])
async def list_workflows(db: Session = Depends(get_db)):
    # Multi-tenancy is handled by the event listener
    return db.query(models.Workflow).all()

@router.get("/{workflow_id}", response_model=WorkflowDetailResponse, dependencies=[require_viewer])
...
@router.post("/{workflow_id}/run", dependencies=[require_editor])
...
@router.get("/{workflow_id}/runs", dependencies=[require_viewer])
